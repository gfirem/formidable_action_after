...
//				check_admin_referer( 'activate_existing_' . $this->_plugin->public_key );
		 ...
		 *                       API error object if failed to install.
		 */
		public function activate_site_with_current_user(
			$license_key = false,
			$redirect = true
		) {
			// Get current logged WP user.
		...
		/**
		 * @author GFireM (@gfirem)
		 * @since  1.1.7.4
		 *
		 * @param FS_User $user
		 * @param string|bool $license_key
		 *
		 * @return object|string If redirect is `false`, returns the next page the user should be redirected to, or the
		 *                       API error object if failed to install.
		 */
		public function register_site(
			...
			return $this->setup_account( $this->_user, $this->_site, false );
		}
